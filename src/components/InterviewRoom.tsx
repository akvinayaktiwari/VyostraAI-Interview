...
import { VyostraLogo, VyostraWordmark } from "./VyostraLogo";
      ...
      <table key={`t${elements.length}`} className="my-2 text-xs border-collapse w-full">
        <tbody>
          {tableRows.map((cells, r) => (
            <tr key={r} className={r === 0 ? "font-semibold text-violet-200" : ""}>
              {cells.map((cell, c) => (
                <td key={c} className="border border-white/10 px-2 py-1 text-violet-100/80">{cell.trim()}</td>
              ))}
            </tr>
          ))}
        </tbody>
    ...
    elements.push(
      <ul key={`l${elements.length}`} className="my-1.5 ml-4 space-y-0.5">
        {listItems.map((item, j) => (
          <li key={j} className="text-violet-100/90 list-disc">{item}</li>
        ))}
      </ul>
    );
    listItems = [];
      ...
      <div className="flex h-screen items-center justify-center px-4">
        <div className="glass w-full max-w-sm rounded-2xl p-8 text-center">
          ...
          <button
            ...
            className="mt-6 rounded-xl bg-zinc-700 px-6 py-2 text-sm text-zinc-300 transition hover:bg-zinc-600"
          >
            Go Back
          </button>
        </div>
      ...
      <div className="flex h-screen items-center justify-center px-4 bg-zinc-950">
        <div className="glass w-full max-w-sm rounded-2xl p-8 text-center">
          ...
          <button
            ...
            className="mt-6 rounded-xl bg-gradient-to-r from-violet-600 to-purple-500 px-8 py-3 text-sm font-medium text-white transition hover:opacity-90"
          >
    ...
    return (
      <div className="flex h-screen items-center justify-center px-4">
        <div className="glass w-full max-w-md rounded-2xl p-8">
          ...
          <div className="mt-4 mb-3 flex items-center gap-2 rounded-xl bg-amber-500/10 border border-amber-500/20 px-3 py-2">
          ...
          {/* Environment Tips */}
          <div className="mt-5 rounded-xl border border-violet-500/20 bg-violet-500/5 p-3.5">
            <p className="text-xs font-semibold text-violet-400 mb-2 uppercase tracking-wider">Before you start</p>
          ...
          <div className="mt-6 space-y-3">
            <label className="flex items-start gap-3 cursor-pointer">
              <input type="checkbox" checked={consentCheck1} onChange={() => setConsentCheck1(!consentCheck1)} className="mt-0.5 h-4 w-4 accent-violet-500" />
              <span className="text-sm text-zinc-300">I understand this session will be recorded</span>
            </label>
            <label className="flex items-start gap-3 cursor-pointer">
              <input type="checkbox" checked={consentCheck2} onChange={() => setConsentCheck2(!consentCheck2)} className="mt-0.5 h-4 w-4 accent-violet-500" />
          ...
          <div className="mt-6 flex gap-3">
            <button
              ...
              className="flex-1 rounded-xl bg-zinc-700 py-2.5 text-sm text-zinc-300 transition hover:bg-zinc-600"
            >
              Decline
            </button>
            <button
              ...
              className="glow-blue flex-1 rounded-xl bg-gradient-to-r from-violet-600 to-purple-500 py-2.5 text-sm font-semibold text-white transition hover:opacity-90 disabled:opacity-40 disabled:shadow-none disabled:cursor-not-allowed"
            >
              I Agree &amp; Continue
            </button>
          </div>
  ...
  if (!isStarted) {
    return (
      <div className="flex h-screen flex-col items-center justify-center gap-8 px-4">
        <VyostraWordmark size={34} tone="light" />
          ...
          {!screenSharing ? (
            <div className="pt-2 border-t border-white/10">
              <button
                ...
                className="w-full rounded-xl bg-violet-500/20 border border-violet-500/30 px-4 py-2.5 text-sm font-medium text-violet-300 hover:bg-violet-500/30 transition-all flex items-center justify-center gap-2"
              >
        ...
        <button
          ...
          className="glow-blue rounded-xl bg-gradient-to-r from-violet-600 to-purple-500 px-10 py-3.5 text-lg font-semibold text-white transition-all hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-40 disabled:shadow-none"
        >
  ...
  return (
    <div className="flex h-screen flex-col">
      ...
      {!micEnabled && isStarted && (
        <div className="absolute top-20 left-1/2 -translate-x-1/2 z-50 bg-red-600 text-white px-4 py-2 rounded-xl text-sm font-medium animate-pulse">
      ...
      <div className="glass flex items-center justify-between px-3 sm:px-6 py-3">
        <div className="flex items-center gap-2 sm:gap-4 min-w-0">
          <VyostraLogo size={22} variant="gradient" className="shrink-0" />
      ...
      <div className="flex flex-col lg:flex-row flex-1 gap-2 sm:gap-3 overflow-hidden p-2 sm:p-3">
        ...
        <div className="flex flex-[5] flex-col gap-2 sm:gap-3">
          ...
          <div className="relative flex-[3] min-h-[150px] sm:min-h-[200px] lg:min-h-0 overflow-hidden rounded-xl sm:rounded-2xl border border-white/5 bg-zinc-900">
            ...
            <div className="absolute bottom-4 left-4 rounded-xl bg-black/60 px-3 py-1.5 text-sm text-white backdrop-blur">
            ...
            <div className="absolute right-4 top-4 flex items-center gap-3">
              <div className="relative" style={{ perspective: "400px" }}>
                {/* 3D animated head */}
                <div
                  className={`w-14 h-14 rounded-full relative transition-all duration-300 ${isAISpeaking ? "shadow-lg shadow-violet-500/40" : ""}`}
                  style={{
                ...
                {isAISpeaking && (
                  <>
                    <div className="pulse-ring absolute inset-0 rounded-full border-2 border-violet-400" />
                    <div className="pulse-ring absolute -inset-1 rounded-full border border-violet-400/50" style={{ animationDelay: "0.5s" }} />
                  </>
                )}
              </div>
                ...
                {isAISpeaking && (
                  <div className="mt-1 flex items-center gap-[2px]">
                    {[0, 1, 2, 3, 4].map((i) => (
                      <div
                        key={i}
                        className="waveform w-[2px] rounded-full bg-violet-400"
          ...
          {currentAIText && (
            <div className="glass rounded-xl px-4 py-3">
              <p className="text-sm leading-relaxed text-violet-300">
                <span className="mr-2 text-xs font-semibold text-violet-500">ALEX:</span>
                {currentAIText}
              </p>
            </div>
          )}
            ...
            <div className="space-y-4">
              {transcript.map((entry, i) => (
                ...
                >
                  ...
                  <div className={`max-w-[85%] rounded-2xl px-4 py-2.5 ${
                    entry.role === "ai"
                      ? "rounded-tl-sm bg-violet-600/20 text-violet-100"
              ...
              {isAIThinking && !isAISpeaking && (
                <div className="flex gap-3 animate-fade-in">
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-violet-600 to-purple-500 text-white text-xs font-bold">
                    A
                  </div>
                  <div className="rounded-2xl rounded-tl-sm bg-violet-600/20 px-4 py-3">
            ...
            {showScrollBottom && (
              <div className="absolute bottom-14 left-1/2 -translate-x-1/2 z-10">
                <button
                  ...
                  className="rounded-full bg-violet-600/90 px-3 py-1.5 text-xs text-white hover:bg-violet-500 transition-all inline-flex items-center gap-1 shadow-lg shadow-black/30"
                >
                  <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            {isAISpeaking && (
              <div className="border-t border-white/5 px-3 py-2">
                <button
                  ...
                  className="w-full rounded-xl bg-zinc-700/50 border border-white/5 px-3 py-1.5 text-xs text-zinc-400 hover:text-white hover:bg-zinc-700 transition-all flex items-center justify-center gap-1.5"
                >
                  <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <div className="border-t border-white/5 p-3">
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 rounded-xl bg-zinc-800/60 border border-white/5 px-3 py-2 text-sm text-zinc-100 placeholder:text-zinc-500 focus:outline-none focus:border-violet-500/50 focus:bg-zinc-800 disabled:opacity-50 disabled:cursor-not-allowed"
                />
                <button
                  onClick={sendChatMessage}
                  disabled={!chatInput.trim() || isAIThinking}
                  className="rounded-xl bg-gradient-to-r from-violet-600 to-purple-500 px-4 py-2 text-sm font-semibold text-white transition-all hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-40"
                  title="Send (Enter)"
                >
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-4 w-4">
      ...
      <div className="flex items-center justify-center gap-2 sm:gap-4 py-3 sm:py-4 flex-wrap">
        ...
        {aiError && !isProcessingRef.current && (
          <button
            ...
            className="rounded-full bg-gradient-to-r from-violet-600 to-purple-500 px-4 py-2.5 text-sm font-semibold text-white transition-all hover:opacity-90 animate-pulse"
          >
            Retry AI
          </button>
        )}
      ...
      <div className="fixed right-4 top-16 z-50 flex flex-col gap-2">
        {proctoringAlerts.map((alert) => (
          <div
            key={alert.id}
            className="glass flex items-center gap-2 rounded-xl border border-yellow-500/20 px-4 py-2"
          ...
          <div className="glass w-full max-w-sm rounded-2xl p-6 text-center">
            ...
            <div className="mt-6 flex justify-center gap-3">
              <button
                ...
                className="rounded-xl bg-zinc-700 px-5 py-2 text-sm text-zinc-200 transition hover:bg-zinc-600"
              >
                Cancel
              </button>
              <button
                ...
                disabled={isEnding}
                className="rounded-xl bg-red-600 px-5 py-2 text-sm font-semibold text-white transition hover:bg-red-500 disabled:opacity-50"
              >
