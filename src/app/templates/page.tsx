            ...
            {templates.map((t) => (
              <div key={t.id} className="card-hover p-6">
                <div className="flex items-start justify-between mb-3">
                  ...
                  {isAdmin && (
                    <div className="flex items-center gap-2">
                      <button onClick={() => openEdit(t)} className="text-xs text-violet-600 hover:text-violet-800 px-2 py-1 rounded hover:bg-violet-50 transition">
                        Edit
                      </button>
                      {!t.is_default && (
                ...
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {["{{candidateName}}", "{{firstName}}", "{{role}}", "{{level}}", "{{duration}}", "{{orgName}}"].map(v => (
                    <span key={v} className="text-[10px] px-1.5 py-0.5 rounded bg-violet-50 text-violet-600 font-mono">{v}</span>
                  ))}
                </div>
              </div>
            ))}
