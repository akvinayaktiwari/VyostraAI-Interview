...
export default function ComparePage() {
  ...
  const SortIcon = ({ field }: { field: SortField }) => (
    <svg
      className={`w-3.5 h-3.5 ml-1 inline-block transition-transform ${sortField === field ? "text-violet-600" : "text-gray-400"} ${sortField === field && sortDir === "asc" ? "rotate-180" : ""}`}
      fill="none"
      viewBox="0 0 24 24"
      stroke="currentColor"
    >
    ...
    <DashboardLayout>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 animate-fade-in-down">
          <h1 className="text-2xl font-extrabold text-gray-900">Compare Candidates</h1>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {selectedInterviews.map((interview, i) => (
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-violet-50 text-violet-700 border border-violet-200 hover:bg-violet-100 transition-colors"
                >
                  <span className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: COLORS[i] }} />
                  {displayName(interview)}
                  <span className="text-violet-400 ml-0.5">{interview.scorecard?.overall}/5</span>
                  <svg className="w-3 h-3 text-violet-400 hover:text-violet-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                  </svg>
                </button>
              ))}
              {selected.length >= 2 && (
                <a href="#comparison" className="text-xs text-violet-600 hover:text-violet-800 font-medium transition-colors ml-auto">
                  Jump to comparison
                </a>
              )}
            </div>
        ...
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-3 mb-4 animate-fade-in-up delay-1">
          ...
          {hasActiveFilters && (
            <button
              ...
              className="text-xs text-violet-600 hover:text-violet-800 font-medium transition-colors whitespace-nowrap"
            >
              Clear filters
            </button>
          )}
        ...
        <div className="animate-fade-in-up delay-2">
          {loading ? (
            ...
            <div className="card p-12 text-center animate-scale-in">
              ...
              <button onClick={clearFilters} className="text-sm text-violet-600 hover:text-violet-800 mt-2 transition-colors">
                Clear filters
              </button>
            </div>
          ) : (
                  ...
                  return (
                    <button
                      ...
                      className={`card-hover p-4 w-full text-left animate-fade-in-up transition-all ${
                        isSelected ? "ring-2 ring-violet-500 ring-offset-1 bg-violet-50/30" : ""
                      ...
                      {paginated.map((interview, idx) => {
                        ...
                        return (
                          <tr
                            ...
                            } ${isSelected ? "border-l-2 border-l-violet-500" : "hover:border-l-2 hover:border-l-violet-500"}`}
                              ...
                              <div className="h-1.5 rounded-full bg-gray-100">
                                <div
                                  className={`h-full rounded-full transition-all duration-700 ease-out ${
                                    isBestDim ? "bg-green-500" : isWorstDim ? "bg-red-500" : "bg-violet-500"
                                  }`}
