...
export default function AISettingsPage() {
  ...
  if (!isAdmin) return (
    <div className="flex min-h-screen bg-white"><Sidebar />
    ...
    <div className="flex min-h-screen bg-white">
      ...
      <div className="flex-1 p-8 max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-extrabold text-gray-900">AI Interviewer Settings</h1>
            <p className="text-sm text-gray-500 mt-1">Customize interviewer behavior, scoring, and persona for your organization</p>
          </div>
          <div className="flex gap-2">
            <button onClick={reset} className="px-4 py-2 text-sm bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200">Reset to defaults</button>
            <button onClick={save} disabled={saving} className="px-5 py-2 text-sm bg-gradient-to-r from-violet-600 to-purple-500 text-white rounded-xl shadow-md shadow-violet-200/50 hover:opacity-90 transition-opacity disabled:opacity-50">
              {saving ? "Saving..." : "Save"}
            </button>
          </div>
        </div>
