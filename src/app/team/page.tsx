            ...
            {users.length === 0 ? (
              <div className="p-16 text-center">
                <svg className="w-28 h-28 mx-auto mb-6 text-gray-200" viewBox="0 0 120 120" fill="none">
                  ...
                  <circle cx="95" cy="75" r="12" fill="#a78bfa" opacity="0.15" stroke="#a78bfa" strokeWidth="2" />
                  <path d="M92 75h6M95 72v6" stroke="#a78bfa" strokeWidth="2" strokeLinecap="round" opacity="0.5" />
                </svg>
                ...
                {users.map((user) => (
                  <div key={user.id} className="flex items-center gap-4 px-6 py-4 hover:bg-gray-50 transition-colors">
                    ...
                    {isAdmin && user.id !== (session?.user as any)?.id && (
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="text-xs text-purple-600 border border-purple-200 hover:bg-purple-50 px-3 py-1.5 rounded-xl transition-colors"
                        >
                          {user.role === "admin" ? "Remove Admin" : "Make Admin"}
                        </button>
                        <button
                          onClick={() => setDeleteTarget({id: user.id, name: user.name})}
                          className="text-xs text-red-500 hover:text-red-700 hover:bg-red-50 px-2 py-1.5 rounded-xl transition-colors"
                        >
                          Delete
                        </button>
                      </div>
                    ...
                    {user.id === (session?.user as any)?.id && (
                      <span className="text-[10px] text-violet-500 font-medium">You</span>
                    )}
                  </div>
                ))}
              </div>
